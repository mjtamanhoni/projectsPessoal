...
router.get('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const result = await horseApi.listarClientes(req.query as Record<string, unknown>);
    res.json(result);
  } catch (error: unknown) {
...
router.post('/', authMiddleware, normalizeCliente, validate(clienteBodySchema), async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await horseApi.salvarClientes(clientes);
    res.json(result);
  } catch (error: unknown) {
    const status = error instanceof Error && 'status' in error ? (error as { status: number }).status : 500;
    res.status(status).json({ error: error instanceof Error ? error.message : 'Erro interno' });
...
router.delete('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await horseApi.excluirCliente(Number(id));
    res.json(result);
  } catch (error: unknown) {
    const status = error instanceof Error && 'status' in error ? (error as { status: number }).status : 500;
    res.status(status).json({ error: error instanceof Error ? error.message : 'Erro interno' });
