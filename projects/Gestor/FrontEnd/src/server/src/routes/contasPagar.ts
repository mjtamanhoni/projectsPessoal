...
router.get('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { dataInicio, dataFim, status, fornecedorId, categoriaId, id: queryId } = req.query as Record<string, string>;
    ...
    const result = await horseApi.listarContasPagar(horseParams);
    let contas = result as ContaPagar[];
    ...
    contas = contas.filter((c) => {
      ...
      if (status === 'pago') return c.pago;
      if (status === 'aberto') return !c.pago;
      if (fornecedorId && c.fornecedorId != null && Number(c.fornecedorId) !== Number(fornecedorId)) return false;
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await horseApi.salvarContasPagar(contas);
    res.json(result);
  } catch (error: unknown) {
    const status = error instanceof Error && 'status' in error ? (error as { status: number }).status : 500;
    res.status(status).json({ error: error instanceof Error ? error.message : 'Erro interno' });
...
router.delete('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await horseApi.excluirContaPagar(Number(id));
    res.json(result);
  } catch (error: unknown) {
    const status = error instanceof Error && 'status' in error ? (error as { status: number }).status : 500;
    res.status(status).json({ error: error instanceof Error ? error.message : 'Erro interno' });
...
router.put('/pagar', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await horseApi.pagarConta({
      id,
...
router.put('/estornar', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const allContas = await horseApi.listarContasPagar() as ContaPagar[];
    const related = allContas.filter(c => c.lancamentoOrigemId === contaId);
    for (const conta of related) {
      await horseApi.excluirContaPagar(conta.id || conta.codigo!);
    }
    const result = await horseApi.estornarContaPagar(contaId);
    res.json(result);
  } catch (error: unknown) {
    const status = error instanceof Error && 'status' in error ? (error as { status: number }).status : 500;
    res.status(status).json({ error: error instanceof Error ? error.message : 'Erro interno' });
