...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        const result = await horseApi_1.horseApi.listarInsumos(req.query);
        res.json(result);
    }
    catch (error) {
...
router.post('/', auth_1.authMiddleware, (0, validate_1.validate)(schemas_1.insumoBodySchema), async (req, res) => {
    try {
        const body = req.body;
        const items = Array.isArray(body) ? body : [body];
        const result = await horseApi_1.horseApi.salvarInsumos(items);
        res.json(result);
    }
    catch (error) {
        const status = error instanceof Error && 'status' in error ? error.status : 500;
...
router.delete('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await horseApi_1.horseApi.excluirInsumo(Number(id));
        res.json(result);
    }
    catch (error) {
        const status = error instanceof Error && 'status' in error ? error.status : 500;
