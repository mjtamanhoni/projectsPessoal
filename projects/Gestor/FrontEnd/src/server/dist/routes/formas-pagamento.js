...
router.get('/', auth_1.authMiddleware, async (req, res) => {
    try {
        const result = await horseApi_1.horseApi.listarFormasPagamento(req.query);
        res.json(result);
    }
    catch (error) {
...
router.post('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await horseApi_1.horseApi.salvarFormasPagamento(items);
        res.json(result);
    }
    catch (error) {
        const status = error instanceof Error && 'status' in error ? error.status : 500;
...
router.delete('/', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const result = await horseApi_1.horseApi.excluirFormaPagamento(Number(id));
        res.json(result);
    }
    catch (error) {
        const status = error instanceof Error && 'status' in error ? error.status : 500;
