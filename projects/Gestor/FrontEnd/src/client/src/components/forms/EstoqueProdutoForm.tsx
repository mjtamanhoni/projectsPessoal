...
type EstoqueProdutoInput = z.infer<typeof estoqueProdutoSchema>;

interface EstoqueProdutoFormProps {
  onSubmit: (data: EstoqueProdutoFabricado) => void;
  onCancel: () => void;
  initial?: EstoqueProdutoFabricado | null;
  produtos: ProdutoFabricado[];
}

export function EstoqueProdutoForm({ onSubmit, onCancel, initial, produtos }: EstoqueProdutoFormProps) {
  const { handleSubmit, formState: { errors }, control } = useForm<EstoqueProdutoInput>({
    resolver: zodResolver(estoqueProdutoSchema),
