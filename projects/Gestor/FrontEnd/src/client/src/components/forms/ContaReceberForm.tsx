...
export function ContaReceberForm({ onSubmit, onCancel, initial, clientes, categorias }: ContaReceberFormProps) {
  ...
  const { handleSubmit, formState: { errors }, control, watch } = useForm<FormValues>({
    defaultValues: initial ? {
      ...
      clienteId: initial.clienteId?.toString() || '',
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4">
            ...
            <div className="px-4 pb-4 space-y-4 border-t border-border-primary pt-3">
              ...
              {gerarParcelasAtivado && watchValor && watchDataVencimento && (
                <div className="text-xs text-text-secondary bg-background-secondary p-3 rounded-lg">
                  <p className="font-medium mb-1">Pre-visualização: {qtdParcelas} parcelas</p>
      ...
      <Controller
        name="clienteId"
        control={control}
        rules={{ required: 'Cliente e obrigatório' }}
        render={({ field }) => (
