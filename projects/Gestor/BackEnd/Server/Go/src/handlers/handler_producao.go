...
func (h *ProducaoHandler) CompraInsumoAtualizar(w http.ResponseWriter, r *http.Request) {
	...
	if isNew && fornecedorID > 0 {
		...
		if errCfg == nil && config != nil {
			...
			if !pago && dataCompra != "" && config.DiasVencimento > 0 {
		...
		if catID > 0 {
			...
			`, ctID, empresaID, usuarioID, fornecedorID,
				descricao, totalValor, vencimento, catID, pago, id).Scan(&ctID)
			if err != nil {
				jsonError(w, err.Error(), http.StatusInternalServerError)
				return
			}
...
func (h *ProducaoHandler) VendaProdutoListar(w http.ResponseWriter, r *http.Request) {
	...
	if id > 0 || r.URL.Query().Get("detalhado") == "1" {
		// Detail list: header + itens (same as compra_insumo pattern)
		...
		query = `SELECT vp.id, vp.empresa_id, vp.cliente_id, vp.data_venda,
			...
			LEFT JOIN public.cliente c ON c.id = vp.cliente_id AND c.empresa_id = vp.empresa_id
			LEFT JOIN LATERAL (
				SELECT COUNT(*) as qtd_itens, SUM(vpi.valor_total) as total_valor
				FROM venda_produto_item vpi
				WHERE vpi.venda_id = vp.id AND vpi.empresa_id = vp.empresa_id
			) agg ON true
			WHERE 1=1`
		if produtoFabricadoID > 0 {
...
func (h *ProducaoHandler) VendaProdutoAtualizar(w http.ResponseWriter, r *http.Request) {
	...
	if isNew {
		...
		_, err = tx.Exec(r.Context(), `
			...
			VALUES ($1,$2,$3,$4::date,0,$5,$6,$7)`,
			id, empresaID, clienteID, dataVenda, observacao, usuarioID, status)
	} else {
		_, err = tx.Exec(r.Context(), `
			UPDATE venda_produto SET cliente_id=$1,
				data_venda=$2::date, observacao=$3, status=$4
			WHERE id=$5 AND empresa_id=$6`,
			clienteID, dataVenda, observacao, status, id, empresaID)
		if err != nil {
			jsonError(w, err.Error(), http.StatusInternalServerError)
			return
		}

		// Restore old items to inventory before deleting
		oldRows, err := tx.Query(r.Context(),
			`SELECT produto_fabricado_id, quantidade FROM venda_produto_item WHERE venda_id = $1 AND empresa_id = $2`,
			id, empresaID)
		if err != nil {
			jsonError(w, err.Error(), http.StatusInternalServerError)
			return
		...
		for oldRows.Next() {
			var o oldItem
			oldRows.Scan(&o.produtoFabricadoID, &o.quantidade)
			oldItems = append(oldItems, o)
		}
		oldRows.Close()
		for _, o := range oldItems {
	...
	for _, rawItem := range itensArr {
		...
		if primeiroNome == "" {
			tx.QueryRow(r.Context(),
				...
				produtoFabricadoID, empresaID).Scan(&primeiroNome)
		}
	...
	tx.Commit(r.Context())
	jsonSuccess(w, map[string]interface{}{"mensagem": "Venda salva com sucesso", "codigo": id, "id": id})
}
...
func (h *ProducaoHandler) EncomendaAtualizar(w http.ResponseWriter, r *http.Request) {
	...
	if isNew {
		...
		vendaID = interface{}(vendaIDAtual)
		if temStatus {
			switch statusNovo {
			case 2:
				if statusAtual == 3 {
				...
				if statusAtual < 2 {
					...
					categoriaReceberID := getInt(header, "categoria_receber_id")
					vid, gerr := h.gerarVendaDeEncomendaTx(r.Context(), tx, empresaID, usuarioID, id, dataVenda, recebido, categoriaReceberID)
					if gerr != nil {
						jsonError(w, gerr.Error(), http.StatusBadRequest)
						return
					}
					vendaID = vid
				}
		...
		if temItens {
			...
			if dataEntrega != "" {
				_, err = tx.Exec(r.Context(), `
					...
					novoStatus, vendaID, dataEntrega, id, empresaID)
			} else {
				_, err = tx.Exec(r.Context(), `
					UPDATE encomenda SET status=$1, venda_id=$2
					WHERE id=$3 AND empresa_id=$4`,
					novoStatus, vendaID, id, empresaID)
			}
			if err != nil {
				jsonError(w, err.Error(), http.StatusInternalServerError)
				return
	...
	jsonSuccess(w, map[string]interface{}{
		"mensagem": "Encomenda salva com sucesso",
		"codigo":   id,
		"id":       id,
		"venda_id": vendaID,
	})
...
func (h *ProducaoHandler) gerarVendaDeEncomendaTx(ctx context.Context, tx pgx.Tx, empresaID, usuarioID, encomendaID int, dataVenda string, recebido bool, categoriaReceberID int) (int, error) {
	...
	vendaID, err := database.GerarID(ctx, tx, empresaID, "venda_produto")
	if err != nil {
		return 0, fmt.Errorf("Erro ao gerar ID da venda: %w", err)
	}
	vendaStatus := 1
	if recebido {
		vendaStatus = 2
	...
	for _, it := range encomendaItens {
		itemID, err := database.GerarID(ctx, tx, empresaID, "venda_produto_item")
		if err != nil {
	...
	if clienteID > 0 {
		...
		var crID int
		crID, err = database.GerarID(ctx, tx, empresaID, "contas_receber")
		if err != nil {
			return 0, fmt.Errorf("Erro ao gerar ID: %w", err)
		}
		err = tx.QueryRow(ctx, `
			INSERT INTO contas_receber (id, empresa_id, usuario_id, cliente_id, descricao,
				valor, data_vencimento, recebido, id_categoria, lancamento_origem_id)
...
// EncomendaGerarVenda finaliza a encomenda (status = 2) e gera a venda de produto
...
func (h *ProducaoHandler) EncomendaGerarVenda(w http.ResponseWriter, r *http.Request) {
	...
	vendaID, err := h.gerarVendaDeEncomendaTx(r.Context(), tx, empresaID, usuarioID, encomendaID, dataVenda, recebido, categoriaReceberID)
	if err != nil {
		jsonError(w, err.Error(), http.StatusBadRequest)
		return
	}

	// Marca a encomenda como finalizada
	_, err = tx.Exec(r.Context(),
