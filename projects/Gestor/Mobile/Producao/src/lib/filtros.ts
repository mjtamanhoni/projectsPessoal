...
export function passaStatus(ativo: boolean | undefined | null, filtro: string): boolean {
  if (filtro === 'todos') return true;
  return (ativo ?? true) === (filtro === '1');
}

export function passaStatusNumero(status: number | undefined | null, filtro: string): boolean {
  if (filtro === 'todos') return true;
